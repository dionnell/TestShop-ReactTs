...
export const RegisterPage = () => {
  ...
  const onSubmit = async (data: RegisterForm) => {
    const isValid = await registerUser(data.fullName, data.email, data.password)

    if (isValid) {
      navigate('/')
      toast.success('Bienvenido a la aplicación')
    } else {
          ...
          <form className="p-6 md:p-8" onSubmit={handleSubmit(onSubmit)}>
            <div className="flex flex-col gap-5">
              ...
              <div className="grid gap-1.5">
                  ...
                  <Input
                    ...
                    {...register("password", {
                      ...
                      validate: {
                        ...
                        hasNumber: (v) =>
                          /\d/.test(v) || "Debe tener al menos un número",
                      },
                    })}
                  />
                  <button
                ...
                {watch("password") && (
                  <ul className="grid grid-cols-2 gap-x-4 gap-y-1 mt-1">
                    {[
                      { label: "Mínimo 6 caracteres", ok: watch("password").length >= 6 },
                      { label: "Una mayúscula", ok: /[A-Z]/.test(watch("password")) },
                      { label: "Una minúscula", ok: /[a-z]/.test(watch("password")) },
                      { label: "Un número", ok: /\d/.test(watch("password")) },
                    ].map(({ label, ok }) => (
                      <li key={label} className={`flex items-center gap-1 text-xs ${ok ? "text-green-600" : "text-gray-400"}`}>
                        <span className={`inline-block h-1.5 w-1.5 rounded-full shrink-0 ${ok ? "bg-green-500" : "bg-gray-300"}`} />
                        {label}
                      </li>
                    ))}
                  </ul>
              ...
              <div className="text-center text-sm">
                ...
                <Link to="/auth/login" className="underline underline-offset-4 font-bold">
                  Iniciar sesión
                </Link>
              </div>
